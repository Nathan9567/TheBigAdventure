...
public class MapParser {
    private static final HashMap<String, ElementMap> encodingMap = new HashMap<>();
...
//    encodingMap.put(elemMatcher.group(1), Element.valueOf(elemMatcher.group(2)));
    ...
    private static ElementMap[][] parseData(String line) {
        Matcher dataMatcher = dataPattern.matcher(line);
        if (!dataMatcher.matches()) {
            throw new IllegalArgumentException("Invalid data format");
        }
        String[] lines = dataMatcher.group(1).split("\n");
        ElementMap[][] data = new ElementMap[lines.length][];
        for (int i = 0; i < lines.length; i++) {
            String[] cells = lines[i].split(" ");
            data[i] = new ElementMap[cells.length];
